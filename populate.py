...
class Populator:
    def __init__(self):
        self.students = []
        self.student_guids = list(Student.objects.all().values_list("GUID", flat=True))

        self.courses = []
        self.course_codes = list(Course.objects.all().values_list("code", flat=True))
    ...
    def random_student_data(self):
        ...
        # while guid in self.student_guids:
        #     print("GUID already exists, generating new one")
        #     guid = f"{random.randint(1000000, 9000000)}{last_name[0]}"
        ...
        current_academic_year = random.randint(start_academic_year, end_academic_year)
    ...
    def random_course_data(self):
        ...
        # while course_code in self.course_codes:
        #     print("Course code already exists, generating a new one.")
        #     course_code = f"COMPSCI{random.randint(1,5)}{random.randint(100, 999)}"

        name = f"Computing Science {course_code[-4:]}"
        lecturer_comments = "".join(lorem_ipsum.paragraphs(random.randint(0,2), False))

        credits = random.choices([10, 20, 40, 60], weights=[0.75, 0.15, 0.075, 0.025])[0]
        if credits == 40:
    ...
    def generate_students(self, n):
        n = n - len(self.student_guids)  # Account for the students that already exist.
        for _ in range(n):
            student = Student(**self.random_student_data)
            self.students.append(student)
            self.student_guids.append(student.GUID)

        print(f"Generated {n} students")
    
    def generate_courses(self, n):
        n = n - len(self.course_codes)  # Account for the courses that already exist.
        i = 0
        courses = []
        while i < n:
            course_data = self.random_course_data
            self.course_codes.append(course_data["code"])
            courses.append(Course(**course_data))
            i += 1
            for j in range(2018, 2023):
                if not course_data["academic_year"] == j and i < n:
    ...
    def populate_database(self):
        ...
        try:
            ...
            print("Transferred students succesfully")
            courses = Course.objects.bulk_create(self.courses, ignore_conflicts=True)
            print("Transferred courses succesfully")
            for course in courses:
                course.enrolled_students.set(students.filter(current_academic_year=course.academic_year))
...
def main():
    ...
    populator.generate_courses(200)
